...
let dBase = firebase.database().ref();
export default class Profile extends Component {
  ...
  componentDidMount(){  // Updates the picture
  ...
  render() {
    ...
    if (this.state.editing == false) {    // FOR NOMRAL PAGE
     ...
     <input type = "file" name = "profpic" accept= "image/" onChange = {e => this.firePost(e)}
      ...
      <div >
        <div className="center">
          ...
          <Row type= "flex" gutter={4}>
            ...
            <input id = "inputfield" type="text" name = "title" value={this.state.title} size= "10" readOnly/*Size is useful*/ />
            </div>
            </Col>

            <Col span ={12}>
            <div id= "userfo" >
            <input id = {this.state.inputclass} type="text" className="header" name = "name" // USERNAME!!! 
            value={this.state.name} onChange= {e => this.handleChange(e)} readOnly={this.state.readmode} maxLength='35'/> 

            <input id = {this.state.inputclass} type="text" name = "position" style={{'textAlign':"center"}} //Inline styling
